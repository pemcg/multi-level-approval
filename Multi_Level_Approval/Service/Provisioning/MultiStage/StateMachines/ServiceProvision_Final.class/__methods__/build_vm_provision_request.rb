...
=begin
  ...
  Author: Kevin Morey <kevin@redhat.com>, Peter McGowan<pemcg@redhat.com>
...
begin
  ...
  when 'automation_task'
    ...
    dialog_options_hash = @task.options[:attrs]['dialog_options_hash']
    dialog_tags_hash = @task.options[:attrs]['dialog_tags_hash']
    @service = $evm.vmdb(:Service, @task.options[:attrs]['service_id'])
  ...
  provision_request_ids = process_builds(dialog_options_hash, dialog_tags_hash)
  $evm.set_state_var(:provision_request_ids, provision_request_ids)
  $evm.set_state_var(:service_id, @service.id)
